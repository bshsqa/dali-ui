...
void NPatchVisual::LoadImages()
{
  ...
  if(mAuxiliaryTextureId == TextureManager::INVALID_TEXTURE_ID && mAuxiliaryUrl.IsValid())
  {
    ...
    else
    {
      ...
      mPreMultipliedAlphaIndex = mImpl->mRenderer.RegisterProperty(UI::Visual::Property::PREMULTIPLIED_ALPHA, PREMULTIPLIED_ALPHA, IsPreMultipliedAlphaEnabled() ? 1.0f : 0.0f);
    }
    ...
    {
      mAuxiliaryResourceStatus = (mAuxiliaryTextureSet && mAuxiliaryTextureSet.GetTextureCount() > 0u) ? UI::Visual::ResourceStatus::READY : UI::Visual::ResourceStatus::FAILED;
    }
  }
}
...
void NPatchVisual::DoSetProperties(const Property::Map& propertyMap)
{
  ...
  Property::Value* auxImageAlpha = propertyMap.Find(UI::DevelImageVisual::Property::AUXILIARY_IMAGE_ALPHA, AUXILIARY_IMAGE_ALPHA_NAME);
  if(auxImageAlpha)
  {
    auxImageAlpha->Get(mAuxiliaryImageAlpha);
  }

  Property::Value* synchronousLoading = propertyMap.Find(UI::ImageVisual::Property::SYNCHRONOUS_LOADING, SYNCHRONOUS_LOADING);
  if(synchronousLoading)
  {
    bool sync = false;
...
Shader NPatchVisual::CreateShader()
{
  ...
  auto fragmentShader = mAuxiliaryResourceStatus == UI::Visual::ResourceStatus::READY ? SHADER_NPATCH_VISUAL_MASK_SHADER_FRAG
                                                                                           : SHADER_NPATCH_VISUAL_SHADER_FRAG;
  auto shaderType     = mAuxiliaryResourceStatus == UI::Visual::ResourceStatus::READY ? VisualFactoryCache::NINE_PATCH_MASK_SHADER
