...
#include <dali-ui-visuals/internal/visuals/animated-image/fixed-image-cache.h>
#include <dali-ui-visuals/internal/visuals/animated-image/rolling-animated-image-cache.h>
#include <dali-ui-visuals/internal/visuals/animated-image/rolling-image-cache.h>
...
void AnimatedImageVisual::DoCreatePropertyMap(Property::Map& map) const
{
  ...
  map.Insert(UI::DevelImageVisual::Property::CURRENT_FRAME_NUMBER, (mImageCache) ? static_cast<int32_t>(mImageCache->GetCurrentFrameIndex()) : -1);
  ...
  if(mMaskingData != nullptr)
  {
    ...
    map.Insert(UI::DevelImageVisual::Property::MASKING_TYPE, mMaskingData->mPreappliedMasking ? DevelImageVisual::MaskingType::MASKING_ON_LOADING : DevelImageVisual::MaskingType::MASKING_ON_RENDERING);
  }
  ...
  switch(index)
  {
    case UI::ImageVisual::Property::PIXEL_AREA:
    {
      ...
      if(DALI_UNLIKELY(mImpl->mRenderer))
      {
        ...
        {
          mPixelAreaIndex = mImpl->mRenderer.RegisterProperty(UI::ImageVisual::Property::PIXEL_AREA, PIXEL_AREA_UNIFORM_NAME, mPixelArea);
        }
      }
      break;
    }
    ...
    case UI::DevelImageVisual::Property::MASKING_TYPE:
    {
      ...
      if(value.Get(maskingType))
      {
        ...
        {
          mMaskingData->mPreappliedMasking = (UI::DevelImageVisual::MaskingType::Type(maskingType) == UI::DevelImageVisual::MaskingType::MASKING_ON_LOADING);
        }
      }
      break;
    }
...
void AnimatedImageVisual::OnInitialize()
{
  ...
  {
    mPixelAreaIndex = mImpl->mRenderer.RegisterUniqueProperty(UI::ImageVisual::Property::PIXEL_AREA, PIXEL_AREA_UNIFORM_NAME, mPixelArea);
  }
  ...
  if(mMaskingData)
  {
    mImpl->mRenderer.RegisterUniqueProperty(UI::ImageVisual::Property::CROP_TO_MASK, CROP_TO_MASK_NAME, static_cast<float>(mMaskingData->mCropToMask));
  }
}
