...
void TextVisual::LoadComplete(bool loadingSuccess, const Dali::UI::Text::TextInformation& textInformation)
{
  ...
  UI::Visual::ResourceStatus resourceStatus = UI::Visual::ResourceStatus::READY; // Initialize resourceStatus here

  if(loadingSuccess && resourceStatus == UI::Visual::ResourceStatus::READY)
  {
    Text::AsyncTextRenderInfo renderInfo = textInformation.renderInfo;
    TilingInfo info(renderInfo.size.width, renderInfo.size.height); // Use renderInfo.size for TilingInfo
    ...
    else
    {
      ...
      // TilingInfo info(verifiedWidth, maxTextureSize); // This line is now redundant due to the earlier insertion

      info.width = verifiedWidth;
      info.height = maxTextureSize; // Set initial height for the first tile
