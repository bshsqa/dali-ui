...
// INTERNAL INCLUDES
#include <dali-ui-visuals/devel-api/visuals/animated-vector-image-visual-signals-devel.h>
...
: Visual::Base(factoryCache, Visual::FittingMode::DONT_CARE, static_cast<UI::Visual::Type>(UI::DevelVisual::ANIMATED_VECTOR_IMAGE)),
  mImageUrl(imageUrl),
  mAnimationData(),
...
void AnimatedVectorImageVisual::DoCreatePropertyMap(Property::Map& map) const
{
  ...
  map.Insert(UI::DevelImageVisual::Property::CURRENT_FRAME_NUMBER, static_cast<int32_t>(mVectorAnimationTask->GetCurrentFrameNumber()));
  map.Insert(UI::DevelImageVisual::Property::TOTAL_FRAME_NUMBER, static_cast<int32_t>(mVectorAnimationTask->GetTotalFrameNumber()));
...
void AnimatedVectorImageVisual::DoSetOnScene(Actor& actor)
{
  ...
  if(mLoadFailed)
  {
    ...
    mRendererAdded = true;
    ResourceReady(Visual::ResourceStatus::FAILED);
  }
  else
  {
...
void AnimatedVectorImageVisual::DoSetOffScene(Actor& actor)
{
  ...
  {
    mImpl->mResourceStatus = Visual::ResourceStatus::PREPARING;
...
void AnimatedVectorImageVisual::OnResourceReady(VectorAnimationTask::ResourceStatus status)
{
  ...
  else
  {
    ...
    if(actor && !mRendererAdded)
    {
      ...
      {
        actor.AddRenderer(mImpl->mRenderer);
        ResourceReady(Visual::ResourceStatus::READY);
      }
      else
      {
        Vector2 imageSize = actor.GetProperty(Actor::Property::SIZE).Get<Vector2>();
