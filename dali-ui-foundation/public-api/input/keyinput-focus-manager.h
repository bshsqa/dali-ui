 ...
 * all the keyboard events first. And if the view doesn't consume the event it is passed to
...
public:
   ...
   * @return A handle to the KeyInputFocusManager.
   */
  static KeyInputFocusManager Get();

  /**
   * Sets keyboard focus for a view.
   * Note: A view can be set to be in focus and still not receive all the key events if another view has over ridden it.
   * As the key input focus mechanism works like a stack, the top most view receives all the key events, and passes on the
   * unhandled events to the views below in the stack. A view in the stack will regain key input focus when there are no more
   * views above it in the focus stack.
   *
   * @pre The View is not in the focus stack. If it is allready present in the top of the stack it results in a no-op, If it is
   * present in the stack but not on the top of the stack, then the view is moved to the top of the focus stack.
...
public: // Signals
   ...
   * Two view parameters are sent as part of this signal, the first being the signal that now has the focus, the second
   * being the one that has lost focus.
   * A callback of the following type may be connected:
   * @code
   *   void YourCallback(View focusGainedView, View focusLostView);
   * @endcode
