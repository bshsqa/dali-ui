...
{
  SetAccessibilityState(state, true);
}

void ViewDataImpl::RemoveAccessibilityState(Accessibility::State state)
{
  SetAccessibilityState(state, false);
}

void ViewDataImpl::SetAccessibilityState(Accessibility::State state, bool enabled)
{
  auto states = GetAccessibilityStates();
  auto mask   = ViewAccessibilityStateToMask(state);
  states      = enabled ? (states | mask) : (states & ~mask);
  SetAccessibilityStates(states);
}
