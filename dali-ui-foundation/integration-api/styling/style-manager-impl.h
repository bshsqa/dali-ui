...
public: // Public API
   ...
   */
  void ApplyThemeStyle(UI::View control);
   ...
   * @param[in] control The control to apply style.
   */
  void ApplyThemeStyleAtInit(UI::View control);

  /**
   * @copydoc UI::StyleManager::ApplyStyle
   */
  void ApplyStyle(UI::View control, const std::string& jsonFileName, const std::string& styleName);
   ...
   */
  const StylePtr GetRecordedStyle(UI::View control);
...
private:
   ...
   */
  void ApplyStyle(UI::Builder builder, UI::View control);
...
private:
  ...
  std::vector<Dali::WeakHandle<UI::View>> mInitializedControlsBeforeAdaptorInit{};  ///< Controls to initialized before the adaptor is initialized
  std::vector<Dali::WeakHandle<UI::View>> mThemeAppliedControlsBeforeAdaptorInit{}; ///< Controls to theme applied before the adaptor is initialized

  bool mAdaptorInitialized : 1; ///< Whether the adaptor has been initialized

  // Signals
  UI::StyleManager::StyleChangedSignalType            mControlStyleChangeSignal; ///< Emitted when the style( theme/font ) changes for the controls to style themselves
  UI::StyleManager::StyleChangedSignalType            mStyleChangedSignal;       ///< Emitted after the controls have been styled
  UI::DevelStyleManager::BrokenImageChangedSignalType mBrokenImageChangedSignal; ///< Emitted after brokenImageChangedSignal
};
